  ...
  return (
    <>
      <header className={`container `+ styles.top}>
        <h1>Hacker news</h1>
        <button className={styles.btn} onClick={refreshComments}>Refresh comments</button>
        <Link className={styles.btn} to={"/"}>Go back</Link>
      </header>
      <main className={"container"}>
        <div className={styles.wrapper}>
          <section className={styles.left}>
            <h2>{title}</h2>
            <div className={styles.comments_title}>Comments: {descendants}</div>
          </section>
          <aside className={styles.right}>
            ...
            <a href={url} className={styles.link} target={"_blank"}>Read more!</a>
          </aside>
        </div>
        {descendants > 0 &&
